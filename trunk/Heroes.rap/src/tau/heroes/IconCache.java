...
public class IconCache
{
	...
	public static void initResources(Display display)
	{
		if (stockImages == null)
		{
			...
			{
				Image image = registerImage(stockImageLocations[i]);
				if (image == null)
				{
					freeResources();
		...
		if (stockCursors == null)
		{
			stockCursors = new Cursor[] {
					null,
					Graphics.getCursor(SWT.CURSOR_WAIT),
					Graphics.getCursor(SWT.CURSOR_SIZEW),
					Graphics.getCursor(SWT.CURSOR_SIZEE), 
					Graphics.getCursor(SWT.CURSOR_CROSS),
					Graphics.getCursor(SWT.CURSOR_SIZEN)};
		}
...
//				image.dispose();
	...
	private static Font createStockFont(Display display, FontDescriptor fontDesctiptor)
	{
		...
		//if (display.loadFont(path))
			return Graphics.getFont(fontDesctiptor.name, fontDesctiptor.size, fontDesctiptor.style);
